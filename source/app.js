(function(){
  ...
  function isEmbeddedInCloudflareAppPreview () {
    if(inIframe()){
      return document.referrer &&  document.referrer.indexOf("cloudflareapps.com") != -1;
    }else{
      return false;
    }
  }
  ...
  if((inIframe() && !isEmbeddedInCloudflareAppPreview()) || (window.frameElement && (window.frameElement.getAttribute('donotembedtrebblewidget') == "true" || window.frameElement.getAttribute('donotembedtrebblewidget') == true))){
    ...
    if (options.whereToAppend !== 'ChooseYourOwn' && options.alwaysVisibible) {
      document.documentElement.className = document.documentElement.className  + " hidebodywhiletrebblesetup";
    }
